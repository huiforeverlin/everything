package com.myself.everything.core.monitor.impl;
...
public class FileWatchImpl implements FileWatch, FileAlterationListener {
    ...
    public FileWatchImpl(FileIndexDao fileIndexDao) {
        ...
        this.monitor = new FileAlterationMonitor(10);//检查间隔：单位 毫秒  遍历时间间隔太长文件变化感知不到，太短性能降低
    }

    @Override
    public void onStart(FileAlterationObserver observer) {
//        observer.addListener(this);//this就是lister对象
    }
            ...
            FileAlterationObserver observer=new FileAlterationObserver(path, new FileFilter() {
                ...
                public boolean accept(File pathname) {
                    ...
                    for(String excludePath:handlePath.getExcludePath()){
                        if(excludePath.startsWith(currentPath)){//如果要排除的是一个目录，则会很方便
                            return false;
                        }
                    }
                    return true;
